...
def get_stock_exchange(stock_code: str) -> str:
    ...
    Args:
        stock_code: 6位股票代码，如 "000001", "600000"
    ...
    # 上交所（SSE）
    if code_prefix in ['600', '601', '603', '605', '688']:
        return 'SSE'
    
    # 深交所（SZSE）
    if code_prefix in ['000', '001', '002', '300']:
        return 'SZSE'
...
def fetch_stock_data_from_vnpy(
    symbol: str,
    start_date: Optional[datetime] = None,
    end_date: Optional[datetime] = None,
    interval: str = "d"
    ...
    try:
        ...
        # 根据股票代码判断交易所
        exchange_str = get_stock_exchange(symbol)
        exchange_map = {
            'SSE': Exchange.SSE,   # 上海证券交易所
            'SZSE': Exchange.SZSE,  # 深圳证券交易所
        }
        exchange_enum = exchange_map.get(exchange_str, Exchange.SSE)
        ...
        print(f"[vnPy] 获取股票数据失败: {e}")
        return None


def get_stock_info_from_vnpy(stock_code: str) -> Optional[Dict[str, Any]]:
    """
    从vnPy获取股票基本信息
    ...
    return {
        'symbol': stock_code,
        'exchange': exchange,
        'name': stock_code,  # vnPy可能不直接提供名称，需要从其他接口获取
    }
