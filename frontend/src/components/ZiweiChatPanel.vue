...
import { startZiweiChatStream } from '../api/ziweiChat';
...
const quickQuestions = computed(() => {
  return ['命宫主星如何？', '事业运势如何？', '财运如何？', '感情婚姻'];
});
...
const handleSend = async () => {
  ...
  stopStream.value = startZiweiChatStream(
    payload,
...
.message-text :deep(strong) { color: #6366f1; }
.message-text :deep(h1), .message-text :deep(h2), .message-text :deep(h3) { 
  margin: 20px 0 12px; color: #1e293b; line-height: 1.4; 
}
.message-text :deep(h1) { font-size: 22px; }
.message-text :deep(h2) { font-size: 18px; border-bottom: 1px solid rgba(99, 102, 241, 0.25); padding-bottom: 8px; }
