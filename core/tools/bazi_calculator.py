...
def calculate_dayun(year: int, month: int, day: int, hour: int, gender: str, bazi_year: Optional[int] = None, sizhu: Dict[str, Any] = None) -> List[Dict[str, Any]]:
    ...
    # 2. "节"：立春、惊蛰、清明、立夏、芒种、小暑、立秋、白露、寒露、立冬、大雪、小寒
    # 3. 顺排：计算到下一个"节"的天数
    # 4. 逆排：计算到上一个"节"的天数
    # 5. 3天=1岁

    from core.tools.solar_terms import get_next_jie, get_prev_jie

    target_date = datetime(year, month, day, hour)

    if is_shun:
        # 顺排：计算到下一个"节"的天数
        next_jie_date = get_next_jie(year, month, day, hour)
        if next_jie_date:
        ...
        # 逆排：计算到上一个"节"的天数
        prev_jie_date = get_prev_jie(year, month, day, hour)
        if prev_jie_date:
